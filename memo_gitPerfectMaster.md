...
HEADの中身は ref:feature （featureブランチを参照）。HEADは現在作業中のブランチへのポインタ。

master,feature,HEADのファイルはリポジトリの中（.git/の下）に記録されている。（.git/HEAD .git/refs/）

ブランチはコミットを指すポインタ。スナップショットを記録してることと相まって、Gitはブランチの作成や切り替え、マージが他のバージョン管理ツールよりも高速。大規模開発において最も使われるツールとなり、普及した。
